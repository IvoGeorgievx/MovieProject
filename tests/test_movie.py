...
class TestMovieSchema(TestRESTAPIBase):
    ...
    def test_create_movie_with_missing_required_fields_expect_raises(self):
        ...
        assert response.json == {'message': {'description': ['Missing data for required field.'],
    ...
    def test_create_movie_that_interferes_with_another_movie_expect_raise(self):
        ...
        response = self.client.post('/create-movie', json=data, headers=headers)
        assert response.status_code == 400
        assert response.json == {'message': 'Hall is occupied during that time.Set another timeframe or choose '
                                            'other hall.'}

    def test_create_movie_with_same_screening_time_but_different_hall_expect_success(self):
        user = UserFactory(role=UserRole.admin)
        token = generate_token(user)
        headers = {"Authorization": f"Bearer {token}"}
    ...
    def test_create_movie_with_non_existing_hall_expect_raise(self):
        ...
        response = self.client.post('/create-movie', json=data, headers=headers)
        assert response.status_code == 400
        assert response.json == {'message': {'hall_id': ['Hall with that ID does not exist.']}}
    ...
    def test_create_movie_with_invalid_ticket_price_expect_raise(self):
        ...
        response = self.client.post('/create-movie', json=data, headers=headers)
        assert response.status_code == 400
        assert response.json == {'message': {'ticket_price': ['Ticket price must be higher than 0.']}}
    ...
    @patch.object(TMDBService, 'get_movie_details', return_value={
    ...
    def test_get_movie_details_expect_success(self, mock_get_movie_details, mock_get_movie_id):
    ...
    @patch.object(TMDBService, 'get_upcoming_movies', return_value={
        "results": [
            {
